...
- [x] **Task 6: Конфиг и секрет ключа Maps** (AC: #2)
  - [x] `config/settings.py`: `GOOGLE_MAPS_API_KEY` + `MAPS_PROVIDER` (env-driven).
  - [x] `.env.example`: добавлен `GOOGLE_MAPS_API_KEY=`.
  - [x] **Прод (2026-06-04, go получен):** выделенный ключ `javi-maps-key` (ограничен Geocoding API) → секрет `javi-google-maps-key` в Secret Manager → роль `secretAccessor` рантайм-SA `488744139718-compute@` → проброс в `deploy.yaml` (`GOOGLE_MAPS_API_KEY`). Billing/Geocoding API уже включены.
...
- 2026-06-04: Прод-ключ Maps провижен (Task 6 закрыт): `javi-maps-key` (Geocoding-only) → секрет `javi-google-maps-key` → SA secretAccessor → `deploy.yaml`. Все AC + Task закрыты. Status → done.
